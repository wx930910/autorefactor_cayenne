...
public class SelectQueryMetadataCacheKeyTest {
	...
	static public ValueObjectType<TestValue, Integer> mockValueObjectType1() {
		ValueObjectType<TestValue, Integer> mockInstance = Mockito.spy(ValueObjectType.class);
		try {
			Mockito.doAnswer((stubInvo) -> {
				throw new UnsupportedOperationException();
			}).when(mockInstance).getTargetType();
			Mockito.doAnswer((stubInvo) -> {
				TestValue object = stubInvo.getArgument(0);
				return Integer.toString(object.v);
			}).when(mockInstance).toCacheKey(Mockito.any());
			Mockito.doAnswer((stubInvo) -> {
				throw new UnsupportedOperationException();
			}).when(mockInstance).toJavaObject(Mockito.any());
			Mockito.doAnswer((stubInvo) -> {
				throw new UnsupportedOperationException();
			}).when(mockInstance).fromJavaObject(Mockito.any());
			Mockito.doAnswer((stubInvo) -> {
				throw new UnsupportedOperationException();
			}).when(mockInstance).getValueType();
	...
	public void createObjects() {
		...
		ValueObjectType testType = SelectQueryMetadataCacheKeyTest.mockValueObjectType1();
